    ...
    >
      ...
      >
        <div className="w-full h-full rounded-r-[2rem] rounded-tr-[12rem] overflow-hidden bg-slate-100 dark:bg-slate-800">
          <img src="https://images.unsplash.com/photo-1555066931-4365d14bab8c?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" alt="Custom Telecom Software" />
        </div>
      </motion.div>
      ...
      >
        <div className="w-full h-full rounded-l-[2rem] rounded-bl-[12rem] overflow-hidden bg-slate-100 dark:bg-slate-800">
          <img src="https://images.unsplash.com/photo-1517694712202-14dd9538aa97?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" alt="Enterprise Platform Engineering" />
        </div>
      </motion.div>
      ...
      <div className="w-full max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-12 relative z-30">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto text-center flex flex-col items-center">
          <SectionBadge text="Enterprise Software Solutions" className="!bg-white dark:!bg-slate-800 !border-brand-100 dark:!border-slate-700 !mb-8" />
          <h1 className="text-4xl sm:text-5xl lg:text-[4rem] font-bold text-slate-900 dark:text-white leading-[1.1] tracking-tight mb-8">
            Tailored <span className="text-brand-500 relative">Telecom Software</span> <br className="hidden md:block" />
            That Powers Enterprise Growth.
          </h1>
          <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 font-medium leading-relaxed mb-10 max-w-2xl">
            From automated billing engines and rate sheet portals to high-throughput CPaaS routing gateways, we build custom software tailored specifically for telecom operators.
          </p>
          <Link to="/contact">
            <Button variant="primary" size="lg" className="shadow-lg shadow-brand-500/30 group">
              Discuss Your Software Needs <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </Link>
        </motion.div>
      </div>
