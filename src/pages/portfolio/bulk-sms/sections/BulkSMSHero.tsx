...
export const BulkSMSHero = () => {
  ...
  return (
    ...
    >
      {/* ========================================= */}
      {/* --- PREMIUM GRADIENT & RADAR EFFECTS --- */}
      {/* ========================================= */}
      ...
      {/* LEFT IMAGE (Full Size Signature Arch) */}
      <motion.div 
      ...
      >
        <div className="w-full h-full rounded-r-[2rem] rounded-tr-[12rem] overflow-hidden bg-slate-100 dark:bg-slate-800">
          <img src="https://plus.unsplash.com/premium_photo-1683865776168-001a4dd1d80b?q=80&w=2532&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" className="w-full h-full object-cover" alt="Sending Bulk SMS" />
        </div>
      </motion.div>

      {/* RIGHT IMAGE (Full Size Signature Arch) */}
      <motion.div 
        initial={{ opacity: 0, x: 50 }} 
        animate={{ opacity: 1, x: 0 }} 
      ...
      >
        <div className="w-full h-full rounded-l-[2rem] rounded-bl-[12rem] overflow-hidden bg-slate-100 dark:bg-slate-800">
          <img src="https://images.unsplash.com/photo-1611746872915-64382b5c76da?q=80&w=1000&auto=format&fit=crop" className="w-full h-full object-cover" alt="SMS Notifications" />
        </div>
      </motion.div>
      ...
      <div className="w-full max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-12 relative z-30">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto text-center flex flex-col items-center">
          <SectionBadge text="Bulk SMS Solutions" className="!bg-white dark:!bg-slate-800 !border-brand-100 dark:!border-slate-700 !mb-8" />
          <h1 className="text-4xl sm:text-5xl lg:text-[4rem] font-bold text-slate-900 dark:text-white leading-[1.1] tracking-tight mb-8">
            A Global <span className="text-brand-500 relative">Bulk SMS</span> Platform <br className="hidden md:block" /> That Grows With You.
          </h1>
          <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 font-medium leading-relaxed mb-10 max-w-2xl">
            Bulk SMS is still one of the fastest and most effective ways to connect with customers. Send messages instantly, reliably, and at scale — ensuring your communication is seen, read, and remembered.
          </p>
          <Link to="/contact">
            <Button variant="primary" size="lg" className="shadow-lg shadow-brand-500/30 group">
              Start Sending SMS <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </Link>
        </motion.div>
      </div>
